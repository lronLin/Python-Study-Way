...
$.get('/house/house_search/' + location.search, function (msg) {
    if(msg.code == '200'){
        // console.log(msg)
        var house_info_html = template('house_list_info', {hlist:msg.house_info})
        $('.houses-list').html(house_info_html)
    }
});

$.get('/house/area_facility', function (msg) {
    if(msg.code == '200'){
        var areas_info_html = template('areas_info', {areas:msg.areas})

        $('.filter-area').html(areas_info_html)
                $(".filter-item-bar>.filter-area").on("click", "li", function(e) {
            if (!$(this).hasClass("active")) {
                $(this).addClass("active");
                $(this).siblings("li").removeClass("active");
