...
def create_stus():
    ...
    for i in range(10):
        # 第一种方式
        # stu = Student()
        # stu.s_name = '小白%s' % random.randrange(1000)
        # stu.s_age = '%s' % random.randrange(20)

        # 第二种方式
        stu = Student('小白%s' % random.randrange(1000),
...
def stu_in_ids():
    ...
    # 模糊搜索
    # select * from student where s_name like '小白%'
    stus = Student.query.filter(Student.s_name.like('%小白%'))
...
@user_blueprint.route('/stu/del_course/<int:id>/', methods=['GET'])
...
# 定义类, 启动包含了对数据处理的get, post, put, patch, delete 请求
...
def roles():
    ...
    if request.method == 'POST':
        return redirect(url_for('roles.html'))
